 ...
 */
SWIFT_EXPORT_FROM(swift_Concurrency)
SWIFT_CC(swift) void swift_task_updateNewChildWithParentAndGroupState(
...
static void swift_taskGroup_attachChildImpl(TaskGroup *group,
   ...
   */
  swift_task_updateNewChildWithParentAndGroupState(child, oldParentStatus,
                                                   group);

  group->addChildTask(child);

  // Release the status record lock, restoring exactly the old status.
  releaseStatusRecordLock(parent, oldParentStatus, recordLockRecord);
}
