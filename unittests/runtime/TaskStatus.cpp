...
TEST(TaskStatusTest, deadline) {
  ...
  withSimpleTask(Storage{47},
    ...
    swift_task_addStatusRecordWithChecks(&recordOne, [&] (ActiveTaskStatus parentStatus) {
    ...
    EXPECT_TRUE(result);
    swift_task_addStatusRecordWithChecks(&recordTwo, [&] (ActiveTaskStatus parentStatus) {
		result = !parentStatus.isCancelled();
		return true;
	});
    EXPECT_TRUE(result);
